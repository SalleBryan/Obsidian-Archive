...
function PdfViewer({ readUrl, title, theme }) {
  ...
  // Arrow-key scrolling forwarded to the PDF iframe
  useEffect(() => {
    const handleKeyDown = (e) => {
      const win = iframeRef.current?.contentWindow;
      if (!win) return;
      if (e.key === "ArrowDown" || e.key === "PageDown") { e.preventDefault(); win.scrollBy(0, 200); }
      else if (e.key === "ArrowUp" || e.key === "PageUp")  { e.preventDefault(); win.scrollBy(0, -200); }
    };
    window.addEventListener("keydown", handleKeyDown);
    return () => window.removeEventListener("keydown", handleKeyDown);
  }, []);
  ...
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", background: bg }}>
      <iframe
        ref={iframeRef}
        src={`${readUrl}#toolbar=1&navpanes=0&scrollbar=1`}
